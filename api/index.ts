...
class TauriPty implements IPty, IDisposable {
  ...
  private _onExit = new EventEmitter2<{
    exitCode: number;
  ...
  constructor(
    ...
    opt?: IWindowsPtyForkOptions,
  );
  constructor(
    pidOrFile: string | number,
    args?: ArgvOrCommandLine,
    opt?: IWindowsPtyForkOptions,
  ) {
  ...
  public get onData(): IEvent<Uint8Array> {
    return this._onData.event;
  }
  public get onExit(): IEvent<{
    exitCode: number;
    signal?: number | undefined;
  }> {
  ...
  resize(columns: number, rows: number): void {
    ...
    this.rows = rows;
    this._init.then(() =>
      invoke("plugin:pty|resize", { pid: this.pid, cols: columns, rows }).catch(
        (e) => {
  ...
  kill(signal?: string | undefined): void {
    this._init.then(() => invoke<string>("plugin:pty|kill", { pid: this.pid }));
  }
  pause(): void {
    throw new Error("Method not implemented.");
  }
  ...
  private async readData() {
    ...
    try {
      for (;;) {
        const data = await invoke<ArrayBuffer>("plugin:pty|read", {
          pid: this.pid,
        });
  ...
  private async wait() {
    ...
    try {
      const exitCode = await invoke<number>("plugin:pty|exitstatus", {
        pid: this.pid,
      });
      this._exitted = true;
